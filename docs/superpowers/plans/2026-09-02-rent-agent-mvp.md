...
- 여러 상품·제도가 함께 검색되면 각 조건을 **해당 상품에만** 귀속시킵니다. 예: 중소기업취업청년 대출의 병역 연령 예외를
...
- "RAG 평가 결과" 섹션: 최종 결과 `eval/results/2026-09-03.md`(6차: F 0.961 / R 0.425 / P 0.851, k=6, n=10) 요약 표 + 1→6차 변화(`eval/results/history.md`) 한 줄 요약 + **평가 한계**(Task 14 Step 3f의 목록 그대로: n=10 작성자 질문, 단일 실행 ±0.05, 판정자=에이전트 모델, P는 k·청크 변경 전후 비교 불가, answer_relevancy 절대값 의미 약함, 테스트셋 튜닝 공개, F는 6차부터 에이전트 ToolMessage 기준)
- "에이전트 동작 예시" 섹션: 진단 질문 1개 → 실행 흐름(에이전트 순서) + 리포트 일부 캡처(텍스트). 통합 테스트 로그 또는 Streamlit 실행 흐름 expander 내용을 그대로 옮긴다(직접 실행해 얻은 실제 출력만 사용).
- "그래프가 보장하는 것" 짧은 절: 두 결정적 후처리(ensure_report, preserve_worker_answer)와 관측 빈도 — ADR-0003 링크
